...
class Rule():
    ...
    def get_rule_name(self, rule_node):
        ...
        try:
            ...
            if (user_comment.type == Rules.Comment):
                # strips comment symbols
                user_comment.content = user_comment.content[1:-1]
    ...
    def generate_function(self):
        ...
        if (self.user_comments is not None):
            string += indent * 1 + "\n"
            string += indent * 1 + self.user_comments.content + "\n"
        string += indent * 1 + '*/\n'
        string += indent * 1 + f"return {self.parse_string}\n}}"
        return string
...
class Grammar_Compiler():
    ...
    def compile(self, node: Node) -> str:
        ...
        fp = importlib.resources.open_text(
            "packratparsergenerator.parser", "rust_core_parser.rs")
    ...
    def get_nodes(self, type):
        ...
        for rule in self.rules:
            if (rule.semantic_instr is None):
                continue
    ...
    def make_rules_enum(self):
        fp = importlib.resources.open_text(
            "packratparsergenerator.grammar_compiler.core_blocks", "rust_rules.rs")
        string = fp.read()
        indent = "    "
        string += indent + "// Following enum values are all autogenerated from grammar file\n"
        for index, rule in enumerate(self.rules):
            string += indent + rule.rule_enum_stmt + ",\n"
        string += "}"
        return string
